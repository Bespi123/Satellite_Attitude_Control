...
q_bodyFrame_calculate = zeros(4,length(w_sensorFrame));
...
%%Simulation
t = measured.t;
q_sim=zeros(4,length(t)+1);
q_sim(:,1) = q_bodyFrame(1,:)';
u=w_bodyFrame;
...
for i = 1:length(t)
    ...
    g1=dt*kinematicEq(t(i), q_sim(:,i), u(:,i));
    g2=dt*kinematicEq(t(i), q_sim(:,i)+0.5.*g1, u(:,i));
    g3=dt*kinematicEq(t(i), q_sim(:,i)+0.5.*g2, u(:,i));
    g4=dt*kinematicEq(t(i), q_sim(:,i)+0.5.*g3, u(:,i));
    q_sim(:,i+1)=q_sim(:,i)+(1/6).*(g1+2.*g2+2.*g3+g4);
end

figure()
subplot(4,1,1)
    plot(t,q_bodyFrame(:,1));grid on; hold on;
    plot(t,q_sim(1,1:end-1)','.')
subplot(4,1,2)
    plot(t,q_bodyFrame(:,2));grid on; hold on;
    plot(t,q_sim(2,1:end-1)','.')
subplot(4,1,3)
    plot(t,q_bodyFrame(:,3));grid on; hold on;
    plot(t,q_sim(3,1:end-1)','.')
subplot(4,1,4)
    plot(t,q_bodyFrame(:,4));grid on; hold on;
    plot(t,q_sim(4,1:end-1)','.')
