...
filename = 'FeedbackControl1.csv'; %%%Test with feedback controller
...
d = diff(measured.t);
t_filtrados = measured.t([true; d ~= 0]);
x_body = [q_bodyFrame,w_bodyFrame'];
x_body_filtrado = x_body([true; d ~= 0],:);
u_body_filtrado = u_bodyFrame([true; d ~= 0],:);
...
measured.x = x_body_filtrado;
measured.u = u_body_filtrado;

%%% Uniform the sample time
[t_uniform, x_uniform] = setSampleTime(t_filtrados,measured.x,dt);
%[~, q_uniform] = setSampleTime(measured.t,q_bodyFrame,dt);
[~, u_uniform] = setSampleTime(t_filtrados,measured.u,dt);
%q_in = q_uniform(450:end,:);
t_in = 0:dt:dt*(length(u_uniform)-1);
...
y = x_uniform(:,5:7);
u = u_uniform;
w_rw_command = (27E-6)\cumtrapz(t_in, u);
...
plot(z(:, :, :)); grid on;  % Plot first input-output pair (t -> q).
