...
addpath('Boskovic controler');
addpath('Cheng controller');
addpath('Dando controler');
addpath('Reaction wheels model');
addpath('Schaub feedback');
...
% J_11 = 0.003351651908532;
% J_22 = 0.002910425765837;
% J_33 = 0.003930855187051;
% J_12 = 4.884184006249866e-06;
% J_23 = 2.560250224819577e-05;
% J_13 = -3.371655548685244e-05;
% cx   = -1.402885514523180e-06; %m
% cy   = -2.661906340358798e-06; %m
% cz   =  2.397039443720151e-07; %m
% m_sat = 1.317; %kg
% alpha = 4.848393490188730e-06;  
% miss_x = 2.605018852742383e-05; 
% miss_y = -1.536704470953186e-05;
% miss_z = 2.364868575802788e-05;

J_tilde=[0.003351651908532,0.002910425765837,0.003930855187051,4.884184006249866e-06,2.560250224819577e-05,2.560250224819577e-05];  %Real inertia tensor (Nm)
J_est=[0.003351651908532,0.002910425765837,0.003930855187051,4.884184006249866e-06,2.560250224819577e-05,2.560250224819577e-05];    %Estimated inertia tensor (Nm)
...
w=pi/180*[0,0,0]';          %Initial body rates (rad/s)    
wrw=pi/180*[0,0,0]';        %Initial rw rates (rad/s)
ui=zeros(3,1);              %Initial control torque (Nm)
x=[q;w;wrw];                %State Space Vector
    
%*************************CONTROL REFERENCES******************************* 
qd=eul2quat(pi/180*[20*cos(0.524*t);zeros(2,length(t))]'); %Desired attitude
qd_dot = [zeros(1,4);diff(qd)./diff(t')]';
wd_Array=zeros(3,n);                           %Desired angular rate array
for i = 1:length(wd_Array)
    wd_Array(:,i) = pinv(xiOperator(qd(i,:)))*2*qd_dot(:,i);
end
qd_Array=qd';
...
gamma=0.05;            %Gain
delta=2;             %Gain
k=0.1;                    %Initial condition for the adaptive gain
...
%********************************Figures***********************************
figure(1)
subplot(3,1,1);plot(t(1:n-1),180/pi*angArray1);hold on;
    plot(t,[20*cos(0.524*t);zeros(2,length(t))]');
...
figure(2)
 subplot(3,1,1);plot(t(1:n-1),180/pi*angArray2); hold on;
    plot(t,[20*cos(0.524*t);zeros(2,length(t))]');
    grid on;
    xlabel('Time(s)');
    ylabel('Euler Angles(deg)');
...
figure(3)
subplot(3,1,1);plot(t(1:n-1),180/pi*angArray3); hold on;
    plot(t,[20*cos(0.524*t);zeros(2,length(t))]');
    grid on; xlabel('Time(s)');
    ylabel('Euler Angles(deg)');
...
figure(4)
subplot(3,1,1);plot(t(1:n-1),180/pi*angArray4); hold on;
    plot(t,[20*cos(0.524*t);zeros(2,length(t))]');
    grid on; xlabel('Time(s)');
    ylabel('Euler Angles(deg)');
